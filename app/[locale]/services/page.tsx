...
export async function generateMetadata({
  ...
  return buildMetadata(locale, "/services", hero.title, hero.description, nav.siteName);
}
    ...
    <main className="min-h-screen bg-white text-gray-900">
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {data.items.map((item, i) => (
            <AnimatedSection key={item.title} variant="fadeUp" delay={i * 0.08}>
              <div className="group p-8 rounded-2xl border border-gray-100 bg-white hover:border-[#cae4f2] hover:shadow-xl hover:shadow-[#009fe1]/10 transition-all duration-300 h-full">
                <div className="w-12 h-12 rounded-xl bg-[#e8f4fb] group-hover:bg-[#cae4f2]/40 transition-colors flex items-center justify-center text-2xl mb-5">
                  {item.icon}
                </div>
      ...
      <section className="px-6 pb-24">
        <AnimatedSection variant="fadeUp">
          <div className="max-w-2xl mx-auto text-center p-12 rounded-2xl bg-[#e8f4fb] border border-[#cae4f2]">
            ...
            <a
              href={`/${locale}/contact`}
              className="inline-block bg-[#009fe1] hover:bg-[#007cb5] text-white font-semibold px-6 py-3 rounded-xl text-sm transition-colors duration-200 shadow-sm"
            >
              {data.cta.button}
            </a>
          </div>
