...
const STATUS_COUNTS_LABELS: Record<string, string> = {
  draft:       "Borradores",
  pending:     "Pendientes",
...
export default async function DashboardPage({
  ...
  const filtered = filterStatus
    ? active.filter((p) => p.status === filterStatus)
    : active;

  const tabLabel = filterStatus ? (STATUS_COUNTS_LABELS[filterStatus] ?? "Proyectos") : "Todos los proyectos";
      ...
      <header className="bg-white border-b border-[#cae4f2] sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/dashboard/papelera" title="Papelera"
            ...
            <Link href="/en/contact" title="Ayuda"
      ...
      <main className="max-w-6xl mx-auto px-6 py-10 flex flex-col gap-10">
        ...
        <div className="rounded-2xl px-8 py-8 flex flex-col gap-6"
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div>
              ...
              <p className="text-white/60 text-sm mt-1">Gestiona tus proyectos desde aquí.</p>
            </div>
            <div className="bg-white/10 rounded-xl px-5 py-3 text-center">
              <p className="text-2xl font-bold text-white">{active.length}</p>
              <p className="text-white/60 text-xs mt-0.5">Total activos</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            {(Object.entries(counts) as [string, number][]).map(([key, count]) => (
              <Link key={key} href={`/dashboard?status=${key}`}
                className="bg-white/10 hover:bg-white/20 rounded-xl px-3 py-2.5 text-center transition-colors cursor-pointer">
                <p className="text-lg font-bold text-white">{count}</p>
                <p className="text-white/60 text-xs">{STATUS_COUNTS_LABELS[key]}</p>
              </Link>
            ))}
          </div>
        </div>
            ...
            {filtered.length === 0 ? (
              <div className="bg-white border border-dashed border-[#cae4f2] rounded-2xl p-12 text-center">
                ...
                <p className="text-gray-400 text-sm font-medium">
                  {filterStatus ? `No hay proyectos en "${STATUS_COUNTS_LABELS[filterStatus] ?? filterStatus}"` : "No tienes proyectos aún"}
                </p>
                {filterStatus && (
                  <Link href="/dashboard" className="text-xs text-[#009fe1] hover:underline mt-1 block">
                    Ver todos →
                  </Link>
                )}
              </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="bg-white rounded-2xl border border-[#cae4f2] p-6 flex flex-col gap-3 hover:shadow-md hover:shadow-[#009fe1]/10 transition-shadow">
            ...
            <Link href="/en/about" className="text-sm font-semibold text-[#009fe1] hover:underline mt-auto">Ver más →</Link>
          </div>
          <div className="bg-white rounded-2xl border border-[#cae4f2] p-6 flex flex-col gap-3 hover:shadow-md hover:shadow-[#009fe1]/10 transition-shadow">
            <div className="w-10 h-10 rounded-xl bg-[#e8f4fb] flex items-center justify-center">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#009fe1" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <Link href="/en/services" className="text-sm font-semibold text-[#009fe1] hover:underline mt-auto">Ver servicios →</Link>
          </div>
        </div>
