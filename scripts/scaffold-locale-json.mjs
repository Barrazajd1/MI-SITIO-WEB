#!/usr/bin/env node
/**
 * scaffold-locale-json.js
 *
 * Lee data/en/<archivo>.json y genera el mismo archivo en los demás idiomas,
 * manteniendo la estructura completa pero marcando los textos como
 * "TODO: translate" para que sepas qué traducir.
 *
 * Los campos que NO son texto (slugs, URLs, fechas, números, íconos, hrefs)
 * se copian tal cual.
 *
 * Uso:
 *   node scripts/scaffold-locale-json.js <archivo>
 *   node scripts/scaffold-locale-json.js blog
 *   node scripts/scaffold-locale-json.js guides
 *   node scripts/scaffold-locale-json.js         ← procesa TODOS los archivos en data/en/
 *
 * Flags:
...
// ─── Campos que se copian sin modificar (no son texto traducible) ─────────────
...
function isTranslatableString(key, value) {
  ...
  // URLs, fechas (YYYY-MM-DD), números puros, emojis solos → no traducir
...
// ─── Recorre el JSON y marca los textos con TODO ──────────────────────────────
function scaffoldValue(key, value) {
  ...
  // números, booleans, null → copiar tal cual
  return value;
}
...
for (const file of files) {
  ...
  for (const locale of locales) {
    ...
    console.log(`✅  ${locale}/${file}  →  ${overwrite && existsSync(destPath) ? "actualizado" : "creado"}`);
    created++;
  }
}

console.log(`\n✨  Listo — ${created} creado(s), ${skipped} omitido(s).\n`);
console.log(`💡  Busca "TODO:" en data/ para ver qué campos necesitan traducción.\n`);
